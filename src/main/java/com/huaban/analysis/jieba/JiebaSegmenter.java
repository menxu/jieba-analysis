...
public class JiebaSegmenter {
    ...
    public List<SegToken> process(String paragraph, SegMode mode, Boolean onlyUseDict) {
        List<SegToken> tokens = process(paragraph, mode);
        if (BooleanUtils.isTrue(onlyUseDict)) {
            tokens = tokens.stream().filter(t -> wordDict.containsWord(t.word)).collect(Collectors.toList());
        }
        return tokens;
    }
